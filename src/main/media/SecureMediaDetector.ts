...
import { MediaManifestParser, HlsVariantRendition } from './MediaManifestParser';
import { DashManifestParser, DashManifestParseResult } from './DashManifestParser';
import { VideoInspector, InspectedVideoMetadata } from './VideoInspector';
import {
...
export class SecureMediaDetector {
  public static async analyze(targetUrl: string, timeoutMs: number = 15000): Promise<ComprehensiveMediaAnalysis> {
    ...
    // 5. Webpage HTML: Fetch and sniffer
    try {
    ...
    if (isMaster) {
      ...
      for (let i = 0; i < variants.length; i++) {
        ...
        if (v.resolution) {
          ...
          // Estimate from bandwidth if resolution header omitted
  ...
  private static async analyzeDirectMedia(
    ...
    const quality: AnalyzedVideoQuality = {
      ...
      frameRate: 30,
      bitrateBps: 6000000,
      bitrateFormatted: sizeBytes && meta.durationSec ? VideoResolutionEngine.formatBitrate(Math.round((sizeBytes * 8) / meta.durationSec)) : 'Direct Stream',
  ...
  private static async analyzeWebpage(
    ...
    // Check embedded HLS or DASH stream links in HTML
    const hlsMatch = html.match(/https?:\/\/[^\s"'<>]+\.m3u8[^\s"'<>]*/i);
    if (hlsMatch) {
      return this.analyzeHlsManifest(hlsMatch[0], pageUrl, tlsResult, timeoutMs);
    }

    const dashMatch = html.match(/https?:\/\/[^\s"'<>]+\.mpd[^\s"'<>]*/i);
    if (dashMatch) {
      return this.analyzeDashManifest(dashMatch[0], pageUrl, tlsResult, timeoutMs);
    }

    // Check direct <video src="...">, <source src="...">
    const directVideoMatch = html.match(/<(?:video|source)[^>]+src=["']([^"']+\.(?:mp4|webm|mkv)[^"']*)["']/i);
    if (directVideoMatch && directVideoMatch[1]) {
      const fullUrl = new URL(directVideoMatch[1], pageUrl).href;
      return this.analyzeDirectMedia(fullUrl, pageUrl, tlsResult, timeoutMs);
    }
    ...
    return {
      ...
      formattedDuration: 'Unknown',
      isProtected,
      protectionReason: isProtected ? 'DRM Encrypted stream detected in webpage.' : 'No openly accessible video streams found.',
      isDownloadable: false,
      availableVideoQualities: [],
      availableAudioTracks: [],
    };
